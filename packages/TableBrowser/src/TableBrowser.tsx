...
import {
  useTableDefinition,
  useConcept,
} from "@react-census/use-table-definition";
...
export function TableBrowser({
  ...
  const filterdConcetps = concepts?.filter((c) =>
    ...
    <Style>
      <div className={"table-browser"}>
        <div className="concepts">
          ...
          />
          <ul>
            {filterdConcetps?.map((c) => (
              <li
                style={{
                  fontWeight: c === selectedConcept ? "bold" : "normal",
                }}
            ...
            <tbody>
              {pagedDeffs &&
                pagedDeffs.map((row) => (
                  <tr
                    className={
                      selectedVariables?.includes(row.id)
                        ? "paleBlueRows selected"
                        : "paleBlueRows"
                    }
                    onClick={() => selectVariable(row.id)}
                  >
                    <td>{row.id}</td>
                    <td>{row.label}</td>
                    <td>{row.group}</td>
